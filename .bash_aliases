...
function md() { pandoc -s -f markdown -t man $1 | groff -T utf8 -man | less; } # enables: md README.md
...
function url_util() { cmd=$(which $1); shift; for a in "$@"; do cmd="$cmd $(echo $1 | sed 's?.*//??;s?[:/].*??')"; shift; done; $cmd; }
alias      dig="url_util dig"
...
function microdate() { microdate_temp="$(date +'%s.%N')"; echo $microdate_temp; echo $microdate_temp $LASTDATE - p|dc; export LASTDATE=$microdate_temp; }
function getrand() { for n in $(seq ${1:-10}); do echo -n ${RANDOM:1:1}; done }
function lso() { ls -l "$@" | awk '{k=0;for(i=0;i<=8;i++)k+=((substr($1,i+2,1)~/[rwx]/)*2^(8-i));if(k)printf(" %0o ",k);print}'; } # ls octal permissions
